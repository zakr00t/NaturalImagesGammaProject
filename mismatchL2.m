function v = mismatchL2(stimParams, rawImageFolder, fValsToUse, RFdata, analogChannels, analogChannelPos)
...
% Currently designed to work with <displaySCNI.m>. Assumes the IN HSV values have already been computed and skips their computation.
    ...
    for i = 1:size(IN, 1)
        ...
        RFcenter = [RFdata.rfStats(analogChannels(analogChannelPos)).meanAzi,RFdata.rfStats(analogChannels(analogChannelPos)).meanEle]; % # RF center (dva)
